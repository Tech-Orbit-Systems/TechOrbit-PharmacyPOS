...
const fs = require("fs");
const { normalizeExpiry } = require("../infrastructure/domain/utils");
...
function prepareLegacyProducts(rows) {
  ...
  const products = rows.map((row, index) => {
    ...
    return { legacySourceId: String(row._id), name: String(row.name).trim(), barcode,
      ...
      expiryDate, active: 1, legacyStockControl: row.stock == null ? null : Number(row.stock) };
  });
...
function importPrepared(db, prepared, metadata) {
  return db.transaction(() => {
    ...
    const addProduct = db.prepare(`INSERT INTO Products
      (barcode, name, category, default_sale_price_minor, minimum_stock, active, created_at, updated_at,
       legacy_source_id, legacy_stock_control, product_type, base_unit)
      VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, 'medicine', 'piece')`);
    const addBatch = db.prepare(`INSERT INTO ProductBatches
      (product_id, supplier_id, batch_number, expiry_date, unit_cost_minor, sale_price_minor, quantity_on_hand,
       received_at, created_at, updated_at, opening_quantity) VALUES (?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)`);
    const addMovement = db.prepare(`INSERT INTO InventoryMovements
      (product_id, batch_id, movement_type, quantity_delta, reference_type, reference_id, occurred_at, note)
      VALUES (?, ?, 'opening', ?, 'legacy_nedb', ?, ?, 'Imported from legacy inventory')`);
    ...
    for (const product of prepared.products) {
      ...
      const productId = Number(addProduct.run(product.barcode, product.name, product.category, product.salePriceMinor,
        product.minimumStock, now, now, product.legacySourceId, product.legacyStockControl).lastInsertRowid);
      db.prepare("INSERT INTO ProductUnits (product_id, unit_name, base_quantity, selling_price_minor, is_default_sale_unit) VALUES (?, 'piece', 1, ?, 1)")
        .run(productId, product.salePriceMinor);
      if (product.openingQuantity > 0) {
        const batchId = Number(addBatch.run(productId, supplierId, `OPENING-${product.legacySourceId}`, product.expiryDate,
          product.salePriceMinor, product.openingQuantity, now, now, now, product.openingQuantity).lastInsertRowid);
    ...
    db.prepare("INSERT INTO LegacyImports (source_file, source_sha256, imported_at, product_count, opening_quantity) VALUES (?, ?, ?, ?, ?)")
      .run(metadata.sourceFile, metadata.sha256, now, prepared.products.length, openingQuantity);
