class SalesQueryService {
  ...
  receipt(id) {
    ...
    if (!sale) return null;
    const saved=this.db.prepare('SELECT payload_json FROM SaleReceiptSnapshots WHERE sale_id=?').get(id);
    if(saved){try{return JSON.parse(saved.payload_json);}catch{throw Error('Saved receipt snapshot is invalid');}}
    ...
    const cashier=this.db.prepare(`SELECT u.id,u.display_name name,r.code roleCode FROM Users u JOIN Roles r ON r.id=u.role_id WHERE u.id=?`).get(sale.created_by)||{id:sale.created_by||null,name:'Unknown cashier',roleCode:'unknown'};
    return {
      version:0,profile:{pharmacyName:'TechOrbit Pharmacy POS',address:null,phone:null,taxRegistration:null,footer:'Thank you for your purchase'},cashier,
      ...
      items:sale.items.map(item=>({lineNumber:item.line_number,productName:item.product_name_snapshot,genericName:item.generic_name_snapshot,saleUnit:item.sale_unit,quantity:item.entered_quantity,originalUnitPriceMinor:item.original_unit_price_minor,unitPriceMinor:item.charged_unit_price_minor,grossMinor:item.gross_minor,lineDiscountMinor:item.line_discount_minor,invoiceDiscountMinor:item.allocated_invoice_discount_minor,gstRateBasisPoints:item.gst_rate_basis_points,gstMinor:item.gst_minor,lineTotalMinor:item.line_total_minor,prescriptionWarning:Boolean(item.prescription_warning),controlledWarning:Boolean(item.controlled_warning),nearExpiryWarning:Boolean(item.near_expiry_warning)})),
