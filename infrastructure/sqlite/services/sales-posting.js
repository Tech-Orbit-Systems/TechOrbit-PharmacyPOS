...
const DEFAULT_RECEIPT_PROFILE={pharmacyName:"TechOrbit Pharmacy POS",address:null,phone:null,taxRegistration:null,footer:"Thank you for your purchase"};
...
class SalesPostingService {
  ...
  postInternal(sale) {
    ...
    const saleResult=this.db.prepare(`INSERT INTO Sales
      (invoice_number,idempotency_key,sold_at,customer_id,customer_name_snapshot,customer_phone_snapshot,payment_method,payment_status,
       ...
       amount_paid_minor,balance_due_minor,due_date,cogs_minor,status,created_by,created_at,request_fingerprint,cash_tendered_minor,cash_change_minor)
      ...
      amountPaidMinor,balanceDueMinor,sale.dueDate||null,sale.createdBy||null,now,sale.requestFingerprint||null,cashTenderedMinor,cashChangeMinor);
    ...
    const profile=this.receiptProfile(),cashier=this.db.prepare(`SELECT u.id,u.display_name name,r.code roleCode FROM Users u JOIN Roles r ON r.id=u.role_id WHERE u.id=?`).get(sale.createdBy)||{id:sale.createdBy||null,name:'Unknown cashier',roleCode:sale.roleCode||'unknown'};
    const warningAcknowledgement=warnings.length&&sale.warningAcknowledged?{warnings,doctorName:sale.doctorName||null,prescriptionReference:sale.prescriptionReference||null,acknowledgedBy:cashier.name,acknowledgedAt:now}:null;
    const receiptSnapshot={version:1,profile,cashier,saleId,invoiceNumber:sale.invoiceNumber.trim(),soldAt,customer:{name:customer?.name||null,phone:customer?.phone||null},payment:{method,status:balanceDueMinor===0?'paid':amountPaidMinor>0?'partial':'credit',amountPaidMinor,balanceDueMinor,dueDate:sale.dueDate||null,cashTenderedMinor,cashChangeMinor},totals:{grossMinor,lineDiscountMinor,invoiceDiscountMinor,taxableMinor,gstMinor,exactTotalMinor,roundingMinor:finalTotalMinor-exactTotalMinor,finalTotalMinor},items:items.map(item=>({lineNumber:item.lineNumber,productName:item.product.name,genericName:item.product.generic_name,saleUnit:item.saleUnit,quantity:item.enteredQuantity,originalUnitPriceMinor:item.originalUnitPriceMinor,unitPriceMinor:item.chargedUnitPriceMinor,grossMinor:item.grossMinor,lineDiscountMinor:item.lineDiscountMinor,invoiceDiscountMinor:item.invoiceDiscountMinor,gstRateBasisPoints:item.product.gst_rate_basis_points,gstMinor:item.gstMinor,lineTotalMinor:item.lineTotalMinor})),warningAcknowledgement};
    this.db.prepare('INSERT INTO SaleReceiptSnapshots(sale_id,payload_json,created_at) VALUES(?,?,?)').run(saleId,JSON.stringify(receiptSnapshot),now);
    this.db.prepare(`INSERT INTO AuditLog(occurred_at,user_id,role_code,action,entity_type,entity_id,new_json,device_id) VALUES (?,?,?,'sale.post','sale',?,?,?)`).run(now,sale.createdBy||null,sale.roleCode||null,String(saleId),JSON.stringify({invoiceNumber:sale.invoiceNumber,finalTotalMinor,amountPaidMinor,balanceDueMinor,cashTenderedMinor,cashChangeMinor,totalCogs,invoiceDiscountType:sale.invoiceDiscountType||null,invoiceDiscountValue:sale.invoiceDiscountValue||0,invoiceDiscountMinor,pricing:items.map(item=>({lineNumber:item.lineNumber,productId:item.product.id,originalUnitPriceMinor:item.originalUnitPriceMinor,chargedUnitPriceMinor:item.chargedUnitPriceMinor,lineDiscountType:item.lineDiscountType||null,lineDiscountValue:item.lineDiscountValue||0,lineDiscountMinor:item.lineDiscountMinor})),batchOverrides:items.filter(item=>item.overrideBatchId).map(item=>({lineNumber:item.lineNumber,productId:item.product.id,suggestedBatchId:item.suggestedBatchId,selectedBatchId:item.overrideBatchId,reason:item.overrideReason,allocations:postedItems.find(posted=>posted.lineNumber===item.lineNumber)?.allocations||[]})),warningAcknowledgement:warnings.length?{warnings,acknowledged:Boolean(sale.warningAcknowledged),acknowledgedBy:sale.warningAcknowledged?sale.createdBy:null,acknowledgedAt:sale.warningAcknowledged?now:null,doctorName:sale.doctorName||null,prescriptionReference:sale.prescriptionReference||null}:null}),sale.deviceId||null);
    return {saleId,invoiceNumber:sale.invoiceNumber,grossMinor,lineDiscountMinor,invoiceDiscountMinor,taxableMinor,gstMinor,exactTotalMinor,roundingMinor:finalTotalMinor-exactTotalMinor,finalTotalMinor,amountPaidMinor,balanceDueMinor,cashTenderedMinor,cashChangeMinor,cogsMinor:totalCogs,items:postedItems,warnings};
  }
  ...
  receiptProfile(){const row=this.db.prepare("SELECT value_json FROM Settings WHERE key='receiptProfile'").get();if(!row)return DEFAULT_RECEIPT_PROFILE;try{const value=JSON.parse(row.value_json);return{pharmacyName:String(value.pharmacyName||DEFAULT_RECEIPT_PROFILE.pharmacyName),address:value.address?String(value.address):null,phone:value.phone?String(value.phone):null,taxRegistration:value.taxRegistration?String(value.taxRegistration):null,footer:String(value.footer||DEFAULT_RECEIPT_PROFILE.footer)}}catch{return DEFAULT_RECEIPT_PROFILE;}}
}
