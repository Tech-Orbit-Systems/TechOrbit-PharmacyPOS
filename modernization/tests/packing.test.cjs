...
test('P030 packing preserves history, stock and sale snapshots; validates units, roles and rollback',async()=>{
 ...
 const input={key:'TO-'+randomUUID(),paymentMethod:'cash',cashTenderedMinor:220100,warningAcknowledged:true,discountMinor:0,items:[{productId:product.id,saleUnit:'test pack',quantity:1}]};const receipt=await g.call('post',input);assert.equal(receipt.totals.finalTotalMinor,220100);assert.equal(stock(),before-20);
